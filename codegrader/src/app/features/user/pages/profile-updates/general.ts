...
"
    ...
    <p-card>
      ...
      <p-table [value]="solved" [paginator]="true" [rows]="10" [responsiveLayout]="'scroll'">
        <ng-template pTemplate="header">
          <tr>
            <th style="width: 100px">ID</th>
            <th>Tên</th>
            <th style="width: 140px">Độ khó</th>
            <th style="width: 180px">Ngày giải</th>
          </tr>
        </ng-template>
        <ng-template pTemplate="body" let-row>
          <tr>
            ...
            <td>{{ row.name }}</td>
            <td>
              <p-tag [value]="row.difficulty" [severity]="tagSeverity(row.difficulty)"></p-tag>
            </td>
            <td>{{ row.solvedAt | date : 'mediumDate' }}</td>
          </tr>
        </ng-template>
      </p-table>
    </p-card>
...
export class GeneralComponent {
  ...
  mediumCount = 20;
  hardCount = 10;
  total = this.easyCount + this.mediumCount + this.hardCount;

  easyPct = Math.round((this.easyCount / this.total) * 100);
  mediumPct = Math.round((this.mediumCount / this.total) * 100);
  hardPct = Math.round((this.hardCount / this.total) * 100);
  solved: SolvedItem[] = [
    { id: 1, name: 'Two Sum', difficulty: 'Easy', solvedAt: new Date().toISOString() },
    {
      id: 2,
    ...
    { id: 3, name: 'Valid Parentheses', difficulty: 'Easy', solvedAt: new Date().toISOString() },
    { id: 4, name: 'Binary Tree Zigzag', difficulty: 'Medium', solvedAt: new Date().toISOString() },
    {
      id: 5,
  ...
  pieData = {
    ...
    datasets: [
      {
        data: [this.counts.Easy, this.counts.Medium, this.counts.Hard],
        backgroundColor: [
          '#22c55e', // Easy - xanh
          '#f59e0b', // Medium - cam
          '#ef4444', // Hard - đỏ
